...
or combinations of other random variables.

::: {.itemize}

* If we roll two dice and let $X$ and $Y$ be the results on each die,
		then the sum is $X+Y$.
...
#| label: fig-runif
#| layout-ncol: 2
#| fig-cap: Two samples from a uniform distribution.
#| fig-subcap: ""
X <- runif(10000, 0,1)
Y <- runif(10000, 0,1)
...
```{r }
#| label: fig-sum-product-uniform-iid
#| layout-ncol: 2
#| fig-cap: 
...
```{r }
#| label: fig-sum-product-uniform-non-iid
#| fig-cap: Adding and Multiplying two uniform random variables that are not independent.
#| fig-subcap:
#|   - 'Sum'
#|   - 'Product'
...
**A.** $\E(3 X + 4) = 3 \E(X) + 4 = 3 \cdot 5 + 4 = `r 3*5 + 4`$

$\Var(3 X + 4) = 3^2 \Var(X) = 3^2 \cdot 2^2 = 36$.  So the standard deviation of 
...
**Q.** Suppose $X$ has a mean of 5 and a standard deviation of 2.  What are the mean
	and standard deviation of $7 - 2 X $?

**A.** $\E(7 - 2X) = \E(- 2 X + 7) = - 2 \E(X) + 7 = -2 \cdot 5 + 7 = `r -2 * 5 + 7`$

$\Var(-2 X + 7) = (-2)^2 \Var(X) = 4 \cdot 2^2 = 16$.  So the standard deviation of 
$7 - 2X$ is $\sqrt{16} = 4$.  Notice that $4 = |-2| \cdot 2$.
::: 
<!-- end example -->
...
**A.**   The mean is $100 + 2 (120) - 3(110) = `r 100 + 2*120 - 3*110`$.

The variance is 
$1^2\cdot 15^2 + 2^2 \cdot 20^2 + (-3)^2 \cdot 25^2 = 
`r  1^2 * 15^2 + 2^2 * 20^2 + (-3)^2 * 25^2`$,
so the standard deviation is 
$\sqrt{ `r  1^2 * 15^2 + 2^2 * 20^2 + (-3)^2 * 25^2`} =
`r  round(sqrt(1^2 * 15^2 + 2^2 * 20^2 + (-3)^2 * 25^2), 1)`$.
::: 
<!-- end example -->
...
X + Y \sim \Norm(10 + 12, \sqrt{2^2 + 4^2}) 
	= \Norm(22, `r round(sqrt(2^2 + 4^2),2)`) \;.
$$
...
Let $X_{1}, X_{2}, \dots X_{12}$ be twelve independent $\Unif(0,1)$-random variables. 
Consider $S = \sum_{i = 1}^{12} X_i$.  
...
as a relatively easy way to simulate normal data given a good pseudorandom number generator
for $\Unif(0,1)$.
::: 
<!-- end example -->
...
their (other) cars might perform in an accident.  
    ...
    * parameter: average amount of force applied to crash dummy's head in all such crashes (not
...
There is a subtlety here that often gets people confused about the interpretation
...
when sampling from a $\Norm(\mu, \sigma)$ population,
...
The Central Limit Theorem tells us that (under certain conditions), the standard 
...
```{r CIsim01, echo = FALSE, fig.width = 8}
...
1. If the population is normal, the confidence intervals achieve
...
2. For modestly sized samples ($15 \le n \le 40$), the $t$-based 
confidence is acceptable as long as the distribution appears 
to be unimodal and is not strongly skewed.
		
3. For large sample size ($n \ge 40$), the $t$-procedure will work acceptably
...
4. Because both the sample mean and the sample variance are sensitive
to outliers, one should proceed with caution when outliers 
are present.

    Outliers that are due to mistakes and can be corrected, should be.  Outliers
...
Use simulations (with 5000 replications) and quantile-quantile plots 
...
gf_qq(~strideRate, data = ex07.37)
df_stats( ~ strideRate, data = ex07.37)
```
...
```{r echo = FALSE}
gf_fun(4 * (x - x^3) * (0 <= x & x <= 1) ~ x, xlim = c(-0.2, 1.2)) |>
  gf_labs(y = "f(x)", x = "x", title = "a pdf")
```
...
can detect that a light source (from a florescent bulb or a computer screen, for example)
...
Create a plot that can be used to visualize the data.  What does your plot suggest
...
Let $\mean X_1$ be the distribution of sample means from a population with mean $\mu_1$ and standard deviation $\sigma_1$.
Let $\mean X_2$ be the distribution of sample means from a population with mean $\mu_2$ and standard deviation $\sigma_2$.
...
$\Norm\left(p, \sqrt{\frac{p(1-p)}{n}}\right)$ to work out the (approximate) sampling distribution for the 
...
a. Use the summary above to compute a 95% confidence interval for the difference in mean extensibility
