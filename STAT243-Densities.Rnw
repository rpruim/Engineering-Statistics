...
The density scale is the same scale that is used by \function{gf_density()}, and it
is the default scale for histograms created using \function{gf_dhistogram()} when
...
\answer
The \function{gf_dist()} function in the \pkg{mosaic} package allows us to 
...
gf_dist("norm", xlab='', ylab='Density') %>%
  gf_refine(scale_x_continuous(breaks = c(-3:3),
                               labels = (c( 
...
Wind <- read.file("https://rpruim.github.io/Engineering-Statistics/data/stob/TwinfallsWind.csv")
head(Wind, 2)
tail(Wind, 2)
...
We can combine the \function{gf_dhistogram()} function with \function{gf_fitdistr()} so we can see how good the fit is graphically.
...
we fit previously.
<<tidy=FALSE>>=
y <- qdata(~speed2, p=probs, data=SmallWind)                     # quantiles from data
...
<<>>=
gf_point( y$p ~ x.gamma )
gf_point( y$p ~ x.weibull )
gf_point( y$p ~ x.normal )
@
\fi
