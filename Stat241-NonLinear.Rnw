...
and 
\verb!^!
have special meanings inside a formula.  \code{I()} stands for \emph{inhibit} special interpretation.
...
yield the commonly used scale for $\beta_0 = \log(A)$, the constant of proportionality.
...
sort of plot.  \R\ can also create plots with transformed scales.  Here are some
...
untransform the response, since the model is a model for $1/C$.  There are two ways
to do that.
<<fig.keep = 'last'>>=
oneOverC <- makeFun(conc.model)
xyplot(concentration ~ time, data = Concentration)
plotFun(1/oneOverC(t) ~ t, add = TRUE)
@
<<fig.keep = 'last'>>=
C <- makeFun(conc.model, transformation = function(x) 1/x)
xyplot(concentration ~ time, data = Concentration)
plotFun(C(t) ~ t, add = TRUE)
@
\end{example}
...
summary(ball.modelA)
plotModel(ball.modelA)
mplot(ball.modelA, w = 1)
@
...
two predictors (\variable{day} and \verb!day^2!) and so will have to wait
...
the \dataframe{Pendulum} data set in the \pkg{fastR22} package.
...
plotFun(f(x) ~ x, col = 'gray50',add = TRUE)
xyplot(period ~ length, data = Pendulum)
...
<<>>=
xyplot(log(pressure) ~ temperature, data = Pressure)
xyplot(log(pressure) ~ log(temperature), data = Pressure)
@
...
<<>>=
xyplot(log(pressure) ~ (273.15 + temperature), data = Pressure)
xyplot(log(pressure) ~ log(273.15 + temperature), data = Pressure)
xyplot(log(pressure) ~ 1/(273.15 + temperature), data = Pressure)
@
...
The data set \dataframe{ACTgpa} (in the \pkg{fastR22} package)
...
The \dataframe{Drag} data set (in the \pkg{fastR22} package) contains the results 
...
plotFun(f2(x) ~ x, add = TRUE, alpha = .4, col = 'red')
plotFun(exp(f3(x)) ~ x, add = TRUE, alpha = .4, col = 'brown')
@
...
plot(model1, w = 1)
xyplot(log(velocity) ~ log(force.drag), data = Drag, groups = height)
xyplot(velocity ~ force.drag, data = Drag, groups = height, 
...
\Rindex{Spheres}%
\myindex{physics|probidx}%
The \dataframe{Spheres} data set (in the \pkg{fastR22} package)
...
\begin{example}
	...
	\begin{enumerate}
		...
		\end{enumerate}
<<>>=
qqmath(resid(power.model), main = "model 1")
