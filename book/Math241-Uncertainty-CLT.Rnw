...
S <- x + y
P <- x * y
gf_histogram( ~ x , main="Sample from Unif(0,1)", binwidth=.05)
gf_histogram( ~ y , main="Another sample from Unif(0,1)", binwidth=.05)
@
<<sum-product-uniform-iid>>=
gf_histogram( ~ S , main="Sum of two iid Unif(0,1) rvs", binwidth=0.1)
gf_histogram( ~ P , main="Product of two iid Unif(0,1) rvs", binwidth=0.05)
@
...
S2 <- x + x
P2 <- x * x 
gf_histogram( ~ S2 , main="Sum of two non-iid Unif(0,1) rvs", binwidth=0.1)
gf_histogram( ~ P2 , main="Product of two non-iid Unif(0,1) rvs", binwidth=0.05)
@
...
\begin{itemize}
\item
when the population distribution is similar to a normal distribution, (unimodal, nearly symmetric, bell-shaped)
\item
...
x <- seq(from=-3, by=0.05, to = 3)
gf_dist(dist="norm", title="Normal and T-distributions") %>%
gf_line( dt(x,df=2) ~ x, col='gray80') %>%
gf_line( dt(x,df=4) ~ x, col='gray60') %>%
gf_line( dt(x,df=8) ~ x, col='gray40') %>%
gf_line( dt(x,df=16) ~ x, col='gray20')
@
...
<<tidy=FALSE>>=
fish3 <- lakemary %>%
  filter(Age==3)  # note the double == here
gf_qq(~Length, data=fish3)
...
<<>>=
gf_boxplot(Length ~ factor(Age), data=lakemary)
gf_point(Length ~ factor(Age), data=lakemary, size=2, alpha=0.3)
@
\end{enumerate}
\end{solution}
...
Paper <- 
  tibble(
    polymer = c(418, 421, 421, 422, 425, 427, 431, 434,
