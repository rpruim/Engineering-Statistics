...
<<fig.keep = "last", fig.height = 3>>=
xyplot( y ~ x, data=someData, xlim=c(0,7), ylim=c(0,7), type = c("p", "g") )
...
<<fig.height = 2.8>>=
xyplot( y ~ x, data=someData, type=c('p','r','g') )
@
...
to measure than OSA.
<<>>=
Eyes <- read.file("http://www.calvin.edu/~rpruim/data/PalpebralFissure.txt")
head(Eyes, 3)
...
\begin{center}
<<osa-scatter>>=
xyplot( OSA ~ palpebral, data=Eyes, type=c('p','r') )
@
\end{center}
...
<<>>=
xyplot( resid(osa.model) ~ palpebral, data=Eyes )
...
% 			\[
% 			\variable{weight} = 
% 			\Sexpr{as.vector(round(coef(model)[1]))} + 
% 			\Sexpr{as.vector(round(coef(model)[2],2))} \cdot \variable{height}
% 			\]
		\item
<<>>=
