...
\end{align*}
where again we evaluate the partial derivatives should be 
evaluated at $B$ and $D$.
...
If we knew the amount of an error, we would correct it and have the exact value.  
...
\begin{align*}
\Parital{F}{k} &= \frac{Pd}{E} 
\\
\Parital{F}{P} &= \frac{kd}{E}
\\
\Parital{F}{d} &= \frac{kP}{E}
\\
\Parital{F}{E} &= \frac{-kPd}{E^2}
\end{align*}

So the estimated amount of fuel consumed (in gallons) is 
\[
