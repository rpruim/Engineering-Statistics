...
\R\ can easily create plots with transformed scales.  Use \function{gf_refine()} with input \texttt{scales_*_log10()} to your call to \texttt{gf_point()}, as detailed in the example below:
<<>>=
x <- 1:10
y <- 3 * x^1.5
gf_point(log(y) ~ log(x))
