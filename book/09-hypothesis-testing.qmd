...
Before we begin to talk about hypothesis testing, let's review the general process of designing and carrying out a statistical experiment. To give the discussion a specific context,
...
should *test* the claim.  The tenor of the conversation changed at 
...
`r round(as.numeric(table(RandomLadies$heads)[6]/100,1))`%
of the time.  But most of the results are quite close to 5 correct.
`r round(sum(table(RandomLadies$heads)[5:7]/100,1))`% of the results are 
4, 5, or 6, for example.
And `r round(sum(table(RandomLadies$heads)[4:8]/100,1))`% of the results 
...
`r round( sum(table(RandomLadies$heads)[10:11]/100), 1 )`% of our simulations). 
...
The null hypothesis is on trial and innocent until proven guilty.  We will
		...
		reject the null (since it appears plausibly compatible with the data).
...
reject the null hypothesis?  Often, a treshhold (called the **significance level**,
and usually denoted $\alpha$) of 0.05 is used.  Sometimes $\alpha = 0.01$ is used
instead.  Basically, this corresponds to a 5% (or 1%) chance of seeing results 
at least as extreme as those found in our data, were the null hypothesis really true.
...
##### Interpretting p-values {-}

When talking about the results of a statistical hypothesis test in a report,
@tbl-p-value-interpretation may be useful.  It relates statements about the
strength of the evidence found in the data with p-values (approximately -- there
are no universal standards and rules here, just guidelines).
...
: Interpreting p-values. How small is small? {#tbl-p-value-interpretation}
...
have applied a hypothesis test, and have rejected the null
...
A pattern strong enough to cause us to reject a null hypothesis of "no
...
confidence intervals, we'll use the t-distribution rather than
...
SE = \frac{s}{\sqrt{n}} = 
\frac{ 
	`r sd(~WeightLoss, data = Controls)`
}{
	\sqrt{ `r nrow(Controls)` } 
}
=
`r sd(~WeightLoss, data = Controls)/sqrt(nrow(Controls))` 
$$
```{r tidy = FALSE}
SE <- 9.108 / sqrt(19); SE
```
...
and from this a p-value, which is the tail probability for a t-distribution with
18 degrees of freedom.  In other words, the p-value gives the probability of
getting a test statistic at least as big as the one we really got, assuming that
the test statistic follows a t-distribution with $n-1$ degrees of freedom.

First, we find the probability of observing a test statistic of $t= `r round(t, 3)`$ or larger.  Then, we have to double this value -- it would be at least as unlikely to see a test statistic of $`r round(-t, 3)`$ or smaller.  Our p-value should give the area under the t-distribution curve for x-values smaller than $`r round(-t, 3)`$ *and* larger than $`r round(t, 3)`$; it's the shaded area in the @fig-tails-of-t:

```{r echo=FALSE, fig.width=7, fig.height=3}
#| label: fig-tails-of-t
#| fig-cap: Tails of a t distribution with 18 degrees of freedom.
#| warning: false
#| message: false
if (FALSE) {
  ...
  xpt(c(-1.876, 1.876), df = 18, return = "plot", alpha = 0.5) |> 
    gf_text(0.1 ~ -1.8, label = ~ "t = -1.876", hjust = 1, vjust = 0, inherit = FALSE) |>
    gf_text(0.1 ~ 1.8, label = ~ "t = 1.876", hjust = 0, vjust = 0, inherit = FALSE) |>
    gf_segment(0 + 0.094 ~ -1.876 + -1.876, inherit = FALSE) |>
    gf_segment(0 + 0.094 ~ 1.876 + 1.876, inherit = FALSE) |>
    gf_refine(scale_fill_manual(values = c('steelblue', 'lightsteelblue', 'steelblue')))
}
```
...
different situations; (or others).  In this case, we cannot consider the
...
both eyes for eight patients who have glaucoma in one eye:

subject     | 1   | 2   | 2   | 4   | 5   | 6   | 7   | 8   |
------------|-----|-----|-----|-----|-----|-----|-----|-----|
...
```{r include=TRUE, fig.width=6, fig.height=2.5}
Glaucoma <- tibble(
  subject = 1:8,
  glaucoma = c(488, 478, 480, 426, 440, 410, 458, 460),
  healthy = c(484, 478, 492, 444, 436, 398, 464, 476),
...
```{r include=FALSE, fig.width=6, fig.height=4.5}
par(cex=1.2)
...
In R, we can compute $\overline{D}$ (the average of $d_i$) and its standard error to obtain the test statistic $t$ and the corresponding p-value:
...
Note that we used 2 * pt(...) because our test statistic $t$ was negative.  If it had been positive, we would have used 2 * (1-pt(...)) instead.  The illustration below may help make this clearer -- the p-value we are computing corresponds to the shaded areas in 
@fig-tails-of-t-2.

```{r echo=FALSE, fig.width=7, fig.height=3}
#| label: fig-tails-of-t-2
#| fig-cap: Tails of a t distribution with 7 degrees of freedom.
#| warning: false
#| message: false
if (FALSE) {
  x <- seq(from=-5, by=0.01, to=5)
  ...
  t <- 1.053
  xpt(c(-t, t), df = 7, return = "plot", alpha = 0.5) |> 
    gf_text(0.255 ~ -t, label = ~ "t = -1.053", hjust = 1, vjust = 0, inherit = FALSE) |>
    gf_text(0.255 ~ t, label = ~ "t = 1.053", hjust = 0, vjust = 0, inherit = FALSE) |>
    gf_segment(0 + 0.24 ~ -t + -t, inherit = FALSE) |>
    gf_segment(0 + 0.24 ~ t + t, inherit = FALSE) |>
    gf_refine(scale_fill_manual(values = c('steelblue', 'lightsteelblue', 'steelblue')))
}
```
...
We can also let R make all the computations for us. Notice that we use the *differences* between pairs as the input data, rather than the raw data itself. We can either
pre-compute this (as we did with `diff` above), or we can compute the differences
in the call to `t.test()`.

```{r}
t.test( ~ diff, data = Glaucoma)
```

```{r}
t.test( ~ (glaucoma - healthy), data = Glaucoma)
```
...
the null hypothesis $H_0$ would be that the average weight loss by
control participants ($\mu_c$) was the same as the average weight loss by
incentivized participants ($\mu_i$).
...
We don't have an obvious indication of unequal variances. (This assumption can
also be tested statistically, although we won't learn how here, and it's
generally not recommended to do such a test prior to running a t-test).  If we
wanted to do the t-test without the equal variance assumption, in R, we would
use:
...
model.  If we can reject that hypothesis, then we have evidence that there
...
In this experiment, the students rigged up a neutrally buoyant balloon and then loaded it 
with different amounts of weight, dropped it, and recorded its terminal velocity.[^ At least that was the plan. There is some evidence in the data that the ballons had not always reached terminal velocity at the time the measurement was taken.]
...
They are testing the hypotheses that $\beta_0 =0$ and that $\beta_1 = 0$.
But we can easily calculate the p-value we want since we have the standard error
and degrees of freedom.


```{r drag-pvalue, tidy = FALSE}
beta1.hat <- 2.051
SE <- 0.05366
t <-  ( beta1.hat - 2 ) / SE; t
2 * pt( - abs(t), df= 40 )
...
**A large p-value does not prove that $a = 2$, 
...
As an example, let's test $H_0: \beta_1 = 2.1$.

```{r more-drag-pvalue, tidy = FALSE}
beta1.hat <- 2.051
SE <- 0.05366
t <-  ( beta1.hat - 2.1 ) / SE; t
2 * pt( - abs(t), df= 40 )
```

This value is also consistent with our data. But $a = 2.5$ is not:


```{r still-more-drag-pvalue, tidy = FALSE}
beta1.hat <- 2.051
SE <- 0.05366
t <-  ( beta1.hat - 2.5 ) / SE; t
2 * pt( - abs(t), df= 40 )
```
...
```{r }
drag.model2 <- nls(force.drag ~ A * velocity^a, data = Drag, start = list(A = 1, a = 2))
summary(drag.model2)
```
...
Our data are consistent with the hypothesis that $a = 2$.
::: 
<!-- end example -->
...
<!-- \authNote{ -->
<!-- 		A test of $\beta_1=0$ in a regression model is often used for *model selection* -- if we cannot reject the null hypothesis of no relationship between the predictor and the response, then the predictor is not really a useful predictor at all. We might want to use (or *select*) a model without that predictor, instead.} -->
...
3.  $H_0: \beta_1 = \beta_{10}$.
    ...
    (The notation above is a bit tricky, $\beta_{10}$ should be read
...
Since the confidence interval for $a$ (i.e., for $\beta_1$) includes 2, 
...
#.  $114 \pm 11$
#.  `r 5.5E-06`.  This gives strong evidence that there is some background radiation
...
#.  $31.5  \pm 1.0$
#.  `r 1.1E-09`.  We have strong evidence that our substance is decaying and 
...
Alternative solution: The wines can be presented in 
...
95 will be  `r  if( p_val < 0.05 ) "outside" else "inside" ` 
the confidence interval because the p-value is
`r  if( p_val < 0.05 ) "less than" else "greater than"` 0.05. 
