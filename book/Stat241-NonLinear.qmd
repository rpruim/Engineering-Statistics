...
the line and obtain our slope without it.
	...
	Exercise~@prob:transformations presents several more examples
...
#.  To obtain a better fit.
	...
	revealed in a scatterplot.  Section~@sec:ladderOfReexpression
...
### The Ladder of Re-expression {#sec-ladder-of-rexpression}
...
from @fig-tukey-buge and @tbl-ladder, which shows four bulge types,
...
same direction as the one in the first quadrant of Figure~@fig:TukeyBulge,
...
::::{.column width="50%"}
```{tikz}
#| label: fig-tukey-buldge
#| fig-cap: Tukey and Mosteller's buldging rule
...
**A.** 
This type of bulge appears in quadrant IV of Figure~@fig:TukeyBulge,
so we can hope to improve the fit by moving 
up the ladder for $x$ or down the ladder for $y$.
As we see in Figure~@fig:TukeyBulgeMany, 
...
that the errors are *independent*.  (See Example~@example:balldropRM
...
a.  Some models cannot be expressed as linear models, even after transformations.
...
<!-- \probNote{Would make a good partial solution for students.}% -->
  ...
  the data?  For each transformation from Exercise~@prob:transformations,
  start with the form
$$
...
Redo @exr-drag after removing this data.
::: 
<!-- end problem -->
