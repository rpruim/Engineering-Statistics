...
We can also use the built-in R function \function{predict()} or \function{fitted()} if we want to make predictions for the original dataset (\function{predict()} can also make predictions for a new dataset that contains new values of the predictor variable -- see example below).
