...
\begin{itemize}
	\item
		...
		\function{gf_dens()}, \function{gf_boxplot()}, etc.) 
	\item
...
The \function{gf_histogram()}
lets you describe the bins in terms of center and width instead of in terms of the number
...
<<compare-ages>>=
gf_histogram( ~ age | sex, HELPrct, binwidth=5)
...
Using \argument{groups} with \function{gf_histogram()} doesn't work so well because
it is difficult to overlay histograms.%
\footnote{The \pkg{mosaic} function
\function{gf_histogram()} does do something meaningful with \argument{groups}
in some situations.}
...
gf_histogram( ~ age, data=HELPrct, col='green')
@
...
When unequal bin sizes are specified, \function{gf_histogram()} by default chooses
the density scale:
...
% gf_histogram( ~ Sepal.Length, data=iris, breaks=c(4,5,5.5,5.75,6,6.5,7,8,9), type='count')
% @
...
<<>>=
gf_histogram(~ i1, data=HELPrct, type="count")
gf_histogram(~ i2, data=HELPrct, type="count")
@
