...
head(Eyes)
x_bar <- mean( ~ palpebral, data = Eyes)
y_bar <- mean( ~ OSA, data = Eyes)
s_x <- sd( ~ palpebral, data = Eyes)
s_y <- sd( ~ OSA, data = Eyes)
r <- cor( palpebral ~ OSA, data = Eyes)
c( x_bar = x_bar, y_bar = y_bar, s_x = s_x, s_y = s_y, r = r )
slope <- r * s_y/s_x
...
estimate ocular surface area (OSA) from palpebral fissure (the horizontal width of the eye
opening in cm) because palpebral fissue is easier to measure than OSA.
<<>>=
...
when doing regression.  If we reverse the roles of \variable{OSA} and \variable{palpebral}
...
As it turns out, the 17th measurement in our data set had a \variable{palpebral} measurement
of 1.2 cm.
<<>>=
Eyes[17, ]
@
...
qqmath( ~ resid(osa.model), data = Eyes)
@
\end{example}
...
are in the \dataframe{ex12.20} data set in the \pkg{Devore7} package.  In that paper,
...
\begin{problem}
	...
	The data are available in the \dataframe{ex12.21} data set in the \pkg{Devore7} package.
...
<<>>=
df_stats( WeightLoss ~ Group, WeightLossIncentive )
coef(summary( model2 ) )
@
