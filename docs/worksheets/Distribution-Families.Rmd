...
* `qunif(q, min, max)` computes the qth quantile/percentile (think: q for quantile)
...
$$
X \sim \Exp(\lambda) \mbox{ on } (0, \infty)
$$
...
**Functions:** `dexp(x, rate)`, `pexp(x, rate)`, `qexp(q, rate)`, `rexp(n, rate)`
...
* $\Prob( \mu -3\sigma \le X \le \mu + 3\sigma) \approx$ 99%
```{r echo = FALSE, results = "hide", message = FALSE}
xpnorm(c(-3, 3))
```
