...
HeatX   # This also lives in the mosaicData package, so read.csv() isn't required
...
<<fig.height = 2.5>>=
xyplot( Q.env ~ m.cold, data = HeatX2, groups = paste("m.hot =", round(m.hot,2)), auto.key = TRUE)
@
\item
...
minute (i.e., \Sexpr{0.5/60} L/sec), then we can compute the uncertainties in 
