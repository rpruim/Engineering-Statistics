...
<<bargraph>>=
gf_bar( ~ substance, data = HELPrct)   # vertical bars
gf_bar(substance ~ ., data = HELPrct)  # horizontal bars
@
...
\subsection{Grouping}
Another way to look at multiple groups simultaneously is placing them "on top of each other"
but distinguishable by color.  Density plots or frequency poloygons work much better than
histograms for overlaying groups.
...
gf_dens( ~ age | substance ~ ., color = ~ sex, data = HELPrct)
@
...
gf_point( Sepal.Length ~ Sepal.Width, color = ~Species, data = iris, alpha = .6, size = .5)
@
...
shape you want.  But there are much better ways to produce documents 
with \R{} graphics in them.
...
\noindent
If you use the \pkg{mosaic} templates, this will be included in the template.
