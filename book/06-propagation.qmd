...
**Uncertainty** is a numerical measure summarizing how large the error *might be*.  There are several different types, or definitions, of uncertainty.  The different definitions of uncertainty have in common that they are all trying to describe a statistical distribution of errors.  
...
Our transformation is $f(x) = x^2$, so $\PartialFrac{f}{x} = f'(x) = 2x$.  
...
uncertainties $u_X$ and $u_Y$.  Then $\PartialFrac{P}{X} = Y$ and $\PartialFrac{P}{Y} =
X$, so
...
$\PartialFrac{R}{R_1}$ and $\PartialFrac{R}{R_2}$.

$$
\begin{aligned}
		\PartialFrac{R}{R_1} &= \frac{ (R_1+R_2)R_2 - (R_1 R_2) }{(R_1 + R_2)^2}
		\\
...
that repeated measurements will all look identical.  
...
our thermometer.
Similarly, if we measure with a ruler with a 1/8 inch scale, we can
...
from your data without further investigation.  You need to know why these values are 
...
There are many measuring devices that provide limited precision so that the 
...
In this case we can estimate a standard uncertainty based on the **uniform distribution**.  
Recall, a uniform
...
marked in mm, if we can tell that a reading is closer to 12.3 mm than it is to 12.4 mm, then we 
...
```{r }
# rectangular: sd = width / sqrt(12) = width / (2 * sqrt(3))
0.4 * 2 / sqrt(12)
0.4 / sqrt(3)
...
The uniform distribution is a more conservative assumption.  The triangle distribution should only
...
Let $f(R) = \pi R^2$.  Then $\PartialFrac{f}{R} = 2\pi R$, so the uncertainty in the area estimation
is 
$$
...
$$
\begin{aligned}
\PartialFrac{F}{k} &= \frac{Pd}{E} 
\\
\PartialFrac{F}{P} &= \frac{kd}{E}
\\
\PartialFrac{F}{d} &= \frac{kP}{E}
\\
\PartialFrac{F}{E} &= \frac{-kPd}{E^2}
\end{aligned}
$$
...
Let $Q = X Y^{-1}$.  So 
$\PartialFrac{Q}{X} = Y^{-1}$ and 
$\PartialFrac{Q}{Y} = -X Y^{-2}$.  From this we get
